...
make test-timing    # the two wall-clock sensitive tests
make test-live      # against the real desktop container
make perf           # time the compose pipeline
```

`make test` is what CI runs, on Linux and macOS. The other three are opt-in, and
`make test` prints them at the end so they are not forgotten.

## The layers

**Unit tests, in `src/`** — 249 of them, next to what they test. Two areas are worth
knowing about.
...
|---|---|---|
| [`pty.rs`](tests/pty.rs) | 12 | the terminal boundary alone: the probe round trip, geometry from `TIOCGWINSZ`, setup and teardown, shared memory, whether frames come out |
| [`resize.rs`](tests/resize.rs) | 10 | size negotiation, and every way a server can answer it |
| [`input.rs`](tests/input.rs) | 8 | keyboard, pointer, clipboard, lock-key correction |
| [`updates.rs`](tests/updates.rs) | 9 | encodings, request pacing, continuous updates, fences, cursor shape |
| [`lifecycle.rs`](tests/lifecycle.rs) | 4 | connecting, losing the connection, leaving the terminal as it was found |
| [`live.rs`](tests/live.rs) | 13 | the same client against a real TigerVNC desktop |
| [`perf.rs`](tests/perf.rs) | 1 | throughput of the compose path |

The four in the middle add a fake VNC server, so a whole session is verified without
Docker. They are split by subject rather than by machinery — the file names say what is
being checked, not which fakes are involved.
...
names its counterpart in a doc comment, in both directions, including the three that
have none and why.
...
would say when the two had drifted apart.

They are deliberately *not* renamed to match each other. Only four are strictly the same
...
**Two timing tests**, `resize::a_drag_is_still_coalesced_into_a_few_requests` and
...
**Fragmented delivery is not tested**, on purpose. noVNC feeds its decoders one byte per
event to break code that assumes a whole message arrived, because it hand-rolls a
receive queue where every decoder must check for buffered input before reading. The rfb
layer here has no bare `read()` calls at all — 28 `read_exact` plus the typed helpers,
over a `BufReader` — and `read_exact` handles a short read by contract, so a fragmenting
fake server would pass on the first run and could never fail.
